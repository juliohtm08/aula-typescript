...
// Tipo que representa um planeta
type Planet = {
  name: string;
  coordinates: [number, number, number, number]; // Coordenadas no formato fixo de 4 números
  status: planetStatus;
  satellites?: string[]; // Satélites opcionais (array de strings)
};
...
function savePlanet(planet: Planet) {
  const planetObj = {
    ...
    satellites: planet.satellites ?? [], // Garante que sempre será um array
  };
...
function updateStatus(name: string, status: keyof typeof planetStatus) {
...
function firstMenuOption() {
  ...
  const coordinates = coordInput.split(',').map(Number) as [
    number,
    number,
    number,
    number
  ...
  const status = prompt(
    'Qual seu status? (habitado, habitavel, inabitavel ou inexplorado)'
  ) as keyof typeof planetStatus;
...
function secondMenuOption() {
  ...
  const status = prompt(
    'Qual o novo status do planeta? (habitado, habitavel, inabitavel ou inexplorado)'
  ) as keyof typeof planetStatus;
